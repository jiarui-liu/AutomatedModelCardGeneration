...
from huggingface_hub.utils import EntryNotFoundError, SoftTemporaryDirectory, validate_hf_hub_args
    ...
    *,
    ...
    for key, value in metadata.items():
        if key == "model-index":
            ...
            if card.data.eval_results is None: # type: ignore
                card.data.eval_results = new_results # type: ignore
                card.data.model_name = model_name # type: ignore
            else:
                existing_results = card.data.eval_results # type: ignore
                ...
                for new_result in new_results:
                    ...
                    if not result_found:
                        card.data.eval_results.append(new_result) # type: ignore
        else:
